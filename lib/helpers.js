...
function dbToMatch(row) {
  let sets;
  try { sets = JSON.parse(row.sets); } catch { sets = []; }
  return {
...
function _isPlayerInMatch(m, playerId) {
...
function hasComeback(playerId, playerMatches) {
  for (const m of playerMatches) {
    ...
    const won = m.isDoubles
      ...
      : m.winnerId === playerId;
    if (!won) continue;
    // Check if player lost the first 2 sets
    let lostFirst2 = true;
    for (let i = 0; i < 2; i++) {
      const s = m.sets[i];
...
function hasRival(playerId, allMatches, allPlayers) {
  ...
  for (const m of allMatches) {
    if (!_isPlayerInMatch(m, playerId)) continue;
    for (const p of allPlayers) {
      if (p.id === playerId) continue;
      if (_isPlayerInMatch(m, p.id)) {
...
function computeAchievements(playerId, stats, eloRating, allMatches, allPlayers) {
  const playerMatches = allMatches.filter(m => _isPlayerInMatch(m, playerId));
